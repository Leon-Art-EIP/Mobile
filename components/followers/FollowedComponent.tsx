...
const FollowedComponent = () => {
  ...
  const [followed, setFollowed] = useState<FollowedType[]>([]);
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (token) {
        ...
        Alert.alert("Erreur lors de la récupération du token. Assurz-vous d'être connecté.");
      }
    } catch (error) {
      console.error('Error fetching user data:', error);
      Alert.alert('Erreur', 'Les données utilisaterus n\'ont pas pu être récupérées.');
    }
  };

  const fetchFollowed = async (userIds: string[]) => {
    try {
      const token = await AsyncStorage.getItem('jwt');
      if (!token) {
        console.error('Token JWT not found. Make sure the user is logged in.');
      ...
      const followedData: FollowedType[] = usersData.map((user) => ({
        id: user._id,
  ...
  useEffect(() => {
    fetchUserData();
    if (userData?.subscriptions) fetchFollowed(userData.subscriptions);
  }, [userData?.subscriptions]);

  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
      { followed.map((follower: FollowedType) => (
        <TouchableOpacity
